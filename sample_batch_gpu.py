...
def main():
    ...
    tmp_cfg.model_path = './trained_models_6625/model930000.pt'
    tmp_cfg.sty_img_path = './reference_image/test/id_0/00019.png'
    tmp_cfg.img_save_path = './result/test/ret-93w/id-0'
    infer(tmp_cfg)
    ...
    tmp_cfg.img_save_path = './result/test/ret-93w/id-1'
    infer(tmp_cfg)
    ...
    tmp_cfg.img_save_path = './result/test/ret-93w/id-5'
    infer(tmp_cfg)
...
def infer(cfg):
    ...
    while len(all_images) * cfg.batch_size < cfg.num_samples:
        model_kwargs = {}
        classes = th.tensor([i for i in char_idx[ch_idx:ch_idx + cfg.batch_size]], device=th.device("cuda"))
        ...
        if cfg.stroke_path is not None:
            chars_stroke = th.empty([0, 32], dtype=th.float32, device="cuda")
            with open(cfg.stroke_path, 'r') as f:
                ...
                for line in lines:
                    ...
                    chars_stroke = th.cat((chars_stroke, th.tensor(char_stroke,device="cuda").reshape([1, 32])), dim=0)
            f.close()
